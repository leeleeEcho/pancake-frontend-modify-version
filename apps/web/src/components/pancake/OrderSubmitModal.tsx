...
const Provider = ({ children }: { children: ReactNode }) => {
  ...
  const onClose = useCallback(() => {
    ...
    onSrcTokenSelected,
    dappTokens,
    lib?.config?.wToken.address,
  ])

  return (
    <SubmitContext.Provider value={{ onClose, state, updateState, swapId: _state.swapId }}>
      {children}
...
const WrapContent = () => {
  ...
  return (
    <StepContent title={`Wrap ${config.nativeToken.symbol}`} indicator message={<AwaitingTxMessage />}>
      <SubmitModalSrcToken />
      <LoadingLottie />
      <SubmitModalDstToken />
    </StepContent>
...
const CreateOrderContent = () => {
  return (
    <StepContent title="Confirm Placing" message={<AwaitingTxMessage />} indicator>
      <SubmitModalSrcToken />
      <LoadingLottie />
      <SubmitModalDstToken />
    </StepContent>
...
const OrderPlacedContent = () => {
  ...
  return (
    <StepContent
      className="twap-order-summary-placed-step"
      title={title}
      indicator={false}
...
const OrderCreatedContent = () => {
  return (
    <StepContent
      className="twap-order-summary-created-step"
...
const ApproveContent = () => {
  ...
  return (
    <StepContent title={`Approve ${inputCurrency?.symbol}`} indicator message={<AwaitingTxMessage />}>
      <SubmitModalSrcToken />
    </StepContent>
  )
}
...
const MinReceived = () => {
  ...
  const { isLimitOrder } = store.useTwapStore((s) => ({
    isLimitOrder: s.isLimitOrder,
  }))
  return (
    <Components.OrderDetails.MinReceived
...
export const isTxRejected = (error: any) => {
  try {
    ...
    console.error('isTxRejected error')
  }
