...
export const StyledBalanceContainer = styled(Styles.StyledRowFlex)<{ hide: number; warning: number }>(
  ({ theme, warning, hide }) => {
    const styles = baseStyles(theme)
    return {
      gap: 2,
...
export const StyledInputContainerChildren = styled('div')<{ focused: number; customBorder?: number }>(
  ({ theme, focused, customBorder }) => {
    const styles = baseStyles(theme)
    const showCustomBorder = customBorder && !focused
    const boxShadow = styles.darkMode
      ? '0px 2px 0px -1px #0000000F inset'
      : '0px 2px 0px -1px rgba(0, 0, 0, 0.06) inset'
...
export const InputContainer = ({
  ...
  return (
    <StyledInputContainer
      className={`twap-input-container ${className}`}
      ref={ref}
      onMouseDown={_onFocus}
      focused={focused ? 1 : 0}
    >
      <StyledInputContainerChildren
        customBorder={customBorder ? 1 : 0}
        focused={focused ? 1 : 0}
        className="twap-input-container-content"
      >
        {children}
      </StyledInputContainerChildren>
    </StyledInputContainer>
