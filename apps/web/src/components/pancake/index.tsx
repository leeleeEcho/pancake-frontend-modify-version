  ...
  return (
    <StyledContainerPadding>
      <StyledTokenPanel>
        ...
        >
          <StyledTokenSelect
            CustomArrow={MdKeyboardArrowDown}
            hideArrow={false}
            isSrc={isSrcToken}
            onClick={onTokenSelectClick}
          />
          <Styles.StyledColumnFlex
            style={{ flex: 1, gap: 0, alignItems: 'flex-end', width: 'auto', overflow: 'hidden' }}
          >
...
const TokenPanelUsd = ({ isSrc }: { isSrc?: boolean }) => {
  ...
  return (
    <StyledUSD
      warning={!isSrc ? 0 : warning ? 1 : 0}
      decimalScale={2}
      isSrc={isSrc}
      hideIfZero={true}
      emptyUi={<StyledEmptyUSD />}
    />
  )
}
    ...
    <StyledPercentSelect show={show ? 1 : 0}>
      {PERCENT.map((p) => {
        const selected = BN(srcAmount || '0').isZero()
          ? false
          : Math.round(percent * 100) === p.value * 100 ||
            (p.value === 1 && BN(maxSrcInputAmount || 0).isEqualTo(srcAmount))
        return (
...
const useTrade = () => {
  ...
  const { useTrade: useTradeInside, connectedChainId, account } = useAdapterContext()
  ...
  const res = useTradeInside!(handleAddress(srcToken?.address), handleAddress(dstToken?.address), amount)
...
const TradeInterval = () => {
  const fillDelayWarning = store.useTwapStore((storeInside) => storeInside.getFillDelayWarning())
  const t = useTwapContext().translations
...
const PricePanelWarning = () => {
  ...
  return (
    <Warning variant="warning">
      <span>
        {' '}
        Limit price is {gainPercent}% lower than market, you are selling at a much lower rate. Please use our{' '}
        <a href="/swap">Swap</a> instead.
      </span>
    </Warning>
  )
}
    ...
    <StyledPricePanelPercent customBorder={!!warning} className="twap-limit-price-panel-percent">
      ...
      <Styles.StyledRowFlex className="twap-limit-price-panel-percent-right">
        <Components.Base.NumericInput
          ...
          loading={isLoading}
          placeholder="0"
          onChange={(value: string) => onPercent(Number(value))}
          value={gainPercent}
        />
...
export const useShowSwapModalButton = () => {
  const { translations: trans } = useTwapContext()
  const { limit } = useAdapterContext()
  const { wrongNetwork, setShowConfirmation, srcAmount } = store.useTwapStore((storeInside) => ({
    maker: storeInside.lib?.maker,
    wrongNetwork: storeInside.wrongNetwork,
    setShowConfirmation: storeInside.setShowConfirmation,
    srcAmount: storeInside.srcAmountUi,
  }))
  const shouldWrap = hooks.useShouldWrap()
  const shouldUnwrap = hooks.useShouldUnwrap()
  const warning = hooks.useFillWarning()
